...
from scipy.stats import bootstrap, gaussian_kde, ConstantInputWarning, NearConstantInputWarning
...
def my_lorentz(x, a, b, c, u):
    return a / (c + np.abs(x - u) ** b)


def my_lorentz_cdf(x, a, b, c, u, min_value, N=50):
    return [np.sum([my_lorentz(y_, a, b, c, u) * (x_ - min_value) / N for y_ in np.linspace(min_value, x_, N)]) for x_
            in x]

def get_mode_with_deviation_with_KDE(data, n_resamples, kernel_bandwidth=None, confidence_level=0.95, weights=None):

    if kernel_bandwidth is None:
        kernel_bandwidth = 'silverman'
    if weights is None:
        mode_avg, kde = get_mode_with_KDE(data, kernel_bandwidth, weights=weights)
        res = bootstrap((data,),
                        lambda x: get_mode_with_KDE(x, kernel_bandwidth=kernel_bandwidth)[0],
                        n_resamples=n_resamples, vectorized=False,  paired=False,method='BCa',
                        confidence_level=confidence_level)
    else:
        mode_avg, kde = get_mode_with_KDE(data, kernel_bandwidth, weights=weights)
        res = bootstrap((data,weights),
                        lambda x,w: get_mode_with_KDE(x, kernel_bandwidth=kernel_bandwidth,  weights=w)[0],
                        n_resamples=n_resamples, vectorized=False,  paired=True, method='BCa',
                        confidence_level=confidence_level)

    mode_std = res.standard_error

    return mode_avg, (res.confidence_interval.low, res.confidence_interval.high), mode_std, kde

def get_mode_with_KDE(data, kernel_bandwidth, weights=None):
    kde = gaussian_kde(data, bw_method=kernel_bandwidth, weights=weights)
    data_sweep = np.linspace(np.min(data), np.max(data), 1000, endpoint=True)

    kde_values = kde(data_sweep)
    mode = data_sweep[np.argmax(kde_values)]
...
def assign_bins(df, df_bins, is_adaptive=True, grid_cols_to_bin=None, adaptive_col_to_bin=None, grid_index_cols=None,
    ...
    grid_index_co = {}
    for col in grid_cols_to_bin:
        grid_index_co[col] = np.digitize(df.loc[:, col].values, grid_bin_edges[col])
        grid_index_co[col][grid_index_co[col]  == len(grid_bin_edges[col])] = 0
        grid_index_co[col]  = grid_index_co[col]  - 1
    ...
    for ij_tuple in product(*[np.unique(grid_index_co[col]) for col in grid_cols_to_bin]):
        ...
        adaptive_bin_indices[adaptive_bin_indices == len(adaptive_bin_edges)] = 0
        adaptive_bin_indices = adaptive_bin_indices - 1
...
def get_bin_edges_adaptive(df, grid_cols=None, adaptive_col='Z', n_grid_bins=(10, 10),
    ...
    adaptive_bin_edges = {}
    mode, _ = get_mode_with_KDE(df[adaptive_col], adaptive_bin_max_size)
    adaptive_bin_min, adaptive_bin_max = 0, n_modes * mode
    ...
    for grid_indices in product(*[np.arange(n) for n in n_grid_bins]):
        ...
        adaptive_bin_edges[grid_indices] = current_bin_edges
...
def get_rolling_basic_statistics(df, window, min_periods, stats_cols, rolling_col, list_of_metrics=None, center=True):
    if list_of_metrics is None:
        list_of_metrics = ['mean', 'std', 'count']
    df_calc = df[[rolling_col] + stats_cols]
    df_calc = df_calc.sort_values(rolling_col)
    df_roll = pd.DataFrame()

    for col in [rolling_col] + stats_cols:
        for metric in list_of_metrics:
            df_roll[f'{col}_{metric}'] = df_calc[col].dropna().rolling(window=window,
                                                                       min_periods=min_periods,
                                                                       center=center).agg(metric)
    for col in [rolling_col] + stats_cols:
        df_roll[f'{col}_sem'] = df_roll[f'{col}_std'] / np.sqrt(df_roll[f'{col}_count'])
...
def get_weighted_average(x_array, weights):
    ...
    if n == 1:
        return x_array[0], 0, 0, 1

    weighted_average = np.average(x_array, weights=weights)
...
def get_maximum_by_quadratic_fit2D(x_values, y_values, f_values, method, bounds=(-np.inf, np.inf),
    ...
    if method == 'fit':
        try:
            ...
            is_maximum = np.all(np.linalg.eigvals(hess_matrix) < - 1e-4)
        except OptimizeWarning as e:
            print(e)
            return np.nan, np.nan, np.nan, np.nan, np.nan
    else:

        X_raw = np.vstack([x_values ** 2,
                       y_values ** 2,
                       x_values,
                       y_values,
                       x_values * y_values]).T
        ...
        X_aug = np.hstack([X, np.ones(shape=(x_values.shape[0], 1))])
        popt_white = np.linalg.inv(X_aug.T @ X_aug + l_regularization.T @ l_regularization) @ X_aug.T @ Y
        sigma_noise = np.linalg.norm(Y - X_aug @ np.array(popt_white)) / np.sqrt(len(x_values) - len(popt_white))
        pcov_white = np.linalg.inv(X_aug.T @ X_aug) * sigma_noise

        a_w, b_w, c_w, d_w, e_w, f_w = popt_white
        pcov = pcov_white
        hess_matrix = np.array([[2 * a_w, e_w], [e_w, 2 * b_w]])
        is_maximum = np.all(np.linalg.eigvals(hess_matrix) < - 1e-4)
        ...
        X_raw_aug = np.hstack([X_raw, np.ones(shape=(x_values.shape[0], 1))])
        # popt_white . X = popt . X_raw  ==> popt = popt_white . X^T . X_raw . (X_raw^T . X_raw) ^ -1
        popt = popt_white @ X_aug.T @ X_raw_aug @ np.linalg.inv(X_raw_aug.T @ X_raw_aug)
        a, b, c, d, e, f = popt


    ea, eb, ec, ed, ee, ef = [pcov[i, i] for i in range(len(popt))]
    x_max = (2 * b * c - d * e) / (e ** 2 - 4 * a * b)
    y_max = (2 * a * d - c * e) / (e ** 2 - 4 * a * b)
    f_max = quadratic_function2D([x_max, y_max],
                                 a, b, c, d, e, f)
    dxda = (4 * b * (2 * b * c - d * e) / (e ** 2 - 4 * a * b) ** 2)
    dxdb = (2 * c * (e ** 2 - 4 * a * b) + 4 * a * (2 * b * c - d * e)) / (e ** 2 - 4 * a * b) ** 2
    dxdc = 2 * b / (e ** 2 - 4 * a * b)
    dxdd = - e / (e ** 2 - 4 * a * b)
    dxde = (-d * (e ** 2 - 4 * a * b) - 2 * e * (2 * b * c - d * e)) / (e ** 2 - 4 * a * b) ** 2

    dyda = (2 * d * (e ** 2 - 4 * a * b) + 4 * b * (2 * a * d - c * e)) / (e ** 2 - 4 * a * b) ** 2
    dydb = 4 * a * (2 * a * d - c * e) / (e ** 2 - 4 * a * b) ** 2
    dydc = - e / (e ** 2 - 4 * a * b)
    dydd = 2 * a / (e ** 2 - 4 * a * b)
    dyde = (- c * (e ** 2 - 4 * a * b) - 2 * e * (2 * a * d - c * e)) / (e ** 2 - 4 * a * b) ** 2

    e_xmax = np.sqrt(dxda ** 2 * ea ** 2
                     + dxdb ** 2 * eb ** 2
                     + dxdc ** 2 * ec ** 2
                     + dxdd ** 2 * ed ** 2
                     + dxde ** 2 * ee ** 2)

    e_ymax = np.sqrt(dyda ** 2 * ea ** 2
                     + dydb ** 2 * eb ** 2
                     + dydc ** 2 * ec ** 2
                     + dydd ** 2 * ed ** 2
                     + dyde ** 2 * ee ** 2)
    ...
    dfde = x_max * y_max
    dfdf = 1
    dfdx = 2 * a * x_max + c + e * y_max
    dfdy = 2 * b * y_max + d + e * x_max

    e_f_max = np.sqrt(dfda ** 2 * ea ** 2
                      + dfdb ** 2 * eb ** 2
                      + dfdc ** 2 * ec ** 2
                      + dfdd ** 2 * ed ** 2
                      + dfde ** 2 * ee ** 2
    ...
    return x_max, y_max, f_max, e_xmax, e_ymax, e_f_max, popt, pcov, is_maximum
...
def get_maximum_vertical_velocity2d(X_array, Y_array, Vz_array, Z_array, n_sigma=0, n_min=50, derivative_epsilon=None,
    ...
    (X_max, Y_max, dZdT_air_max,
     e_X_max, e_Y_max, e_dZdT_air_max,
     popt, pcov, is_maximum) = get_maximum_by_quadratic_fit2D(X_array[mask],
                                                              Y_array[mask],
                                                              Vz_array[mask],
                                                              bounds=bounds,
                                                              method=method
    ...
    if debug and Z_avg > 300:
        ...
        debug_plot_decomposition_get_maximum_vertical_velocity2d(X_array, Y_array, Vz_array, mask, popt,
                                                                 X_max, Y_max, dZdT_air_max)
    if not is_maximum:
        return np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, Z_avg, popt, pcov, N

    return X_max, Y_max, dZdT_air_max, e_X_max, e_Y_max, e_dZdT_air_max, Z_avg, popt, pcov, N
...
def get_correlation_differences_from_sample(field, correlation_span, scale=(1, 1), n_dimensions=2,
    ...
    if isinstance(correlation_span, numbers.Number):
        correlation_span = [correlation_span] * n_dimensions
    correlation_span_index = [round(c / s) for c,s in zip(correlation_span, scale)]
    ...
    if n_dimensions == 2:
        ...
        correlation_span_index_nb = List()
        [correlation_span_index_nb.append(x) for x in correlation_span_index]
        corr = get_correlation_differences_from_sample_new3D(field=field,
                                                             correlation_index_span=correlation_span_index_nb,
                                                             plot_debug=plot_debug)
    array_of_deltas = []
    for s, cs in zip(scale,correlation_span_index):
        array_of_deltas.append(np.arange(-cs, cs + 1) * s)

    deltas_mg = np.meshgrid(*array_of_deltas, indexing='ij')
    return corr, deltas_mg
...
def get_correlation_differences_from_sample_new2d(field, correlation_index_span, plot_debug=False):
    ...
    sample_field = field[
                   correlation_index_span[0]:-correlation_index_span[0],
                   correlation_index_span[1]:-correlation_index_span[1]]
    output_shape = (2 * correlation_index_span[0] + 1,
                    2 * correlation_index_span[1] + 1)
    ...
    if plot_debug:
        ...
        ax[0].contourf(delta_x, delta_y, field, levels=levels, norm=norm)
        # delta_x, delta_y = np.meshgrid(np.arange(sample_field.shape[0]) + correlation_index_span[0],
        #                                np.arange(sample_field.shape[1]) + correlation_index_span[1], indexing='ij')
        ...
        sample_delta_y = delta_y[
                         correlation_index_span[0]:-correlation_index_span[0],
                         correlation_index_span[1]:-correlation_index_span[1]]
        ax[0].hlines(y=[sample_delta_y.min(),
                        sample_delta_y.max()],
                     xmin=[sample_delta_x.min()] * 2,
                     xmax=[sample_delta_x.max()] * 2,
                     color='r', linestyles='dashed')
        ax[0].vlines(x=[sample_delta_x.min(), sample_delta_x.max()],
                     ymin=[sample_delta_y.min()] * 2, ymax=[[sample_delta_y.max()]] * 2,
                     color='r', linestyles='dashed')

        ax[1].contourf(sample_delta_x,
                       sample_delta_y,
                       sample_field, levels=levels, norm=norm)
        delta_x, delta_y = np.meshgrid(np.arange(output_shape[0]),
                                       np.arange(output_shape[1]), indexing='ij')
...
@njit(parallel=True)
def get_correlation_differences_from_sample_new3D(field, correlation_index_span, debug=False, plot_debug=False):
    #scale = np.array([scale, scale, scale])
    sample_field = field[
                   correlation_index_span[0]:-correlation_index_span[0],
                   correlation_index_span[1]:-correlation_index_span[1],
                   correlation_index_span[2]:-correlation_index_span[2]]
    output_shape = (2 * correlation_index_span[0] + 1,
                    2 * correlation_index_span[1] + 1,
                    2 * correlation_index_span[2] + 1)
    corr = np.empty(shape=output_shape)
    for delta_x in prange(output_shape[0]):
        for delta_y in prange(output_shape[1]):
...
def get_all_permutated_rms(df1, df2, col_to_rms, col_to_permute, list_to_permute, n_permutations=np.inf):
    list_of_rms = []

    current_diff = []
    for i_turbulence_gt, (p_1, p_2) in enumerate(zip(list_to_permute, list_to_permute)):
        current_df1 = df1[df1[col_to_permute] == p_1]
        current_df2 = df2[df2[col_to_permute] == p_2]
        current_merge = pd.merge(current_df1, current_df2, on='delta_R', suffixes=('_x', '_y'))
        current_diff += (current_merge[f'{col_to_rms}_x'] - current_merge[f'{col_to_rms}_y']).tolist()
    null_rms = np.array(current_diff)
    null_rms = np.sqrt(np.nanmean(null_rms ** 2))

    for permutations_gt in permutations(list_to_permute):
        current_diff = []
        for i_turbulence_gt, (p_1, p_2) in enumerate(zip(list_to_permute, permutations_gt)):
            current_df1 = df1[df1[col_to_permute] == p_1]
            current_df2 = df2[df2[col_to_permute] == p_2]
            current_merge = pd.merge(current_df1, current_df2, on='delta_R', suffixes=('_x', '_y'))
            current_diff += (current_merge[f'{col_to_rms}_x'] - current_merge[f'{col_to_rms}_y']).tolist()
        ...
        list_of_rms.append(current_rms)
        should_break = len(list_of_rms) > n_permutations
        if should_break:
            break
    return list_of_rms, null_rms
...
def diff_correlation_function(a,b):
    return np.abs(a - b)


def get_binned_diff_correlation_function(X_array, f_array, deltaR, method='diff'):
    corr_func = diff_correlation_function if method == 'diff' else correlation_function
    n_points = X_array.shape[0]
    distance_matrix = np.zeros((n_points, n_points))
    for i_x, x in enumerate(X_array):
        for i_y, y in enumerate(X_array[i_x:]):
            distance_matrix[i_x, i_x + i_y] = np.linalg.norm(x - y)
    distance_matrix = distance_matrix + np.triu(distance_matrix, 1).T

    bins = distance_matrix // deltaR
    corr_array = []
    for i_bin in np.arange(np.max(bins) + 1):

        current_indices = np.argwhere(bins == i_bin)
        if len(current_indices) == 0:
            continue
        n_points = current_indices.shape[0]
        current_corr = [np.mean(corr_func(f_array[current_indices[:,0]], f_array[current_indices[:,1]])),
                        np.std(corr_func(f_array[current_indices[:,0]], f_array[current_indices[:,1]])),
                        np.mean(distance_matrix[bins == i_bin]),
                        np.std(distance_matrix[bins == i_bin]),
                        n_points]
        corr_array.append(current_corr)
    corr_array = np.array(corr_array)

    return corr_array


def get_rms_and_pearson(x_array, y_array, **kwargs):
    bad_values_mask = np.isnan(x_array)
    bad_values_mask = np.logical_or(bad_values_mask, np.isnan(y_array))
    bad_values_mask = np.logical_or(bad_values_mask, x_array == np.inf)
    bad_values_mask = np.logical_or(bad_values_mask, y_array == np.inf)

    good_x_array = x_array[~bad_values_mask]
    good_y_array = y_array[~bad_values_mask]
    n = good_y_array.size
    # Root Mean Squared
    try:
        rms = np.sqrt(np.nanmean((good_x_array - good_y_array) ** 2))
    except RuntimeWarning as e:
        print(e)
        rms = np.nan
    ...
    # Pearson Correlation
    try:
        if good_y_array.size < 2:
            correlation = np.nan
            p_value = np.nan
        else:
            res = stats.pearsonr(good_x_array, good_y_array, **kwargs)
            correlation, p_value = res.statistic, res.pvalue
    except ConstantInputWarning:
        correlation = np.nan
        p_value = np.nan
    except NearConstantInputWarning:
        correlation = np.nan
        p_value = np.nan

    return rms, correlation, p_value, n