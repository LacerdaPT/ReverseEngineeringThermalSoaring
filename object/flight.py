...
class Trajectory:
    def __init__(self, position, control_parameters=None):
        ...
        # if np.all([v is None for v in self.control_parameters['glide'].values()]):
        #     self.control_parameters['glide'] = False
    ...
    def get_flight_mode(self, N=5):
        ...
        elif self.control_parameters['glide'] and (current_point['t'] > self.control_parameters['glide']['trigger']['time']):
            flight_mode = 'glide'
        else:
    ...
    def get_control(self, dt, air_velocity_obj=None):
        ...
        if np.isclose(last_point['t'] - self.last_control_time, self.control_parameters['general_args']['period']):
            ...
            if flight_mode == 'glide':
                bank_angle_new, flight_mode, thermalling_mode = self.get_control_glide(dt)
            elif flight_mode == 'thermalling':
                bank_angle_new, flight_mode, thermalling_mode = self.get_control_thermal()
            else: #flight_mode == 'out_of_thermal':
                bank_angle_new, flight_mode = self.go_back_to_thermal(dt)
                thermalling_mode = ''
        else:
            flight_mode = self.get_flight_mode(N=self.control_parameters['general_args']['N_out_of_thermal'])
        ...
        self.set_thermal_core_estimation(1, 50)
    ...
    def go_to_point(self, target_position, dt, method='delta_v', debug=False):
        ...
        return bank_angle_new, 'glide', None
    ...
    def go_back_to_thermal(self, dt):
        last_point = self.get_last_N_point()
        ...
        bank_angle_new, _, _ = self.go_to_point(target_point, dt=dt)  # = self.get_bank_angle_from_go_to_point(target_position=target_point)
    ...
    def get_control_glide(self, dt):
        if self.gliding_target is None:
            distance = 1000
            theta = self.control_parameters['glide']['direction']
            self.gliding_target = np.array([distance * np.cos(theta), distance * np.sin(theta)])
        return self.go_to_point(self.gliding_target, dt=dt)

    def get_control_thermal(self):
        if self.should_explore():
