...
df_best = pd.read_csv(os.path.join(path_to_csv, 'best_set_of_wing_loadings.csv'), index_col=False)

df_best.rename(columns={'WL_mode_avg': 'WL_best'}, inplace=True)
...
df_pearson = df_pearson[df_pearson['c1'] == '012']


my_norm = Normalize(df_best['WL_best'].min(), df_best['WL_best'].max())

fig_correlations, ax_corr = plt.subplots(2,2, figsize=(figsize_multiplier * 4.75, figsize_multiplier * 4.75),
...
for i_c, (_, row) in enumerate(df_pearson.iterrows()):
    ...
    df_merge = pd.merge(df_1, df_2, on=['bird_name', 'loss_percentile'],
                          suffixes=('_1', '_2'))
    df_merge = pd.merge(df_merge, df_best,on=['bird_name', 'loss_percentile'],)
    ...
    current_ax_corr.scatter(df_merge['WL_mode_avg_1'].values, df_merge['WL_mode_avg_2'].values,
                            c=df_merge['WL_best'], norm=my_norm, cmap='viridis_r')
...
plt.colorbar(ScalarMappable(norm=my_norm, cmap='viridis_r'), ax=ax_corr[1])

if save:
    plt.draw()
    plt.draw_all()
    fig_correlations.savefig(os.path.join('/home/pedro/Documents/Research/Publications/Science/figures/main_text/storks_behaviour/include', f'wl-correlations-012-345.png'))
    fig_correlations.savefig(os.path.join('/home/pedro/Documents/Research/Publications/Science/figures/main_text/storks_behaviour/include', f'wl-correlations-012-345.svg'))
