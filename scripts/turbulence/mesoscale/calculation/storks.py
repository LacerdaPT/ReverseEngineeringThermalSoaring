...
air_component = 'air'
base_path = Path(os.path.join(root_path, 'synthetic_data/from_atlasz/newdata/storks'))
glob_string = '*/decomposition/individual_bins/bin_z_size=10/optimized/n_resamples=1000/final/reconstructed'
save_folder = os.path.join(root_path, 'results/turbulence/storks/mesoscale_test', air_component)
...
for i_t, current_path in enumerate(base_path.glob(glob_string)):
    for datatype in ['dec']:
        ...
        if ((not df_all_thermals_avg.empty)
                and (not df_all_thermals_avg[np.all(df_all_thermals_avg[list(label_dict.keys())].astype(str) == [str(v) for v in label_dict.values()],
                                                    axis=1)].empty)):
