...
list_of_sizes = [30]
#/home/pedro/PycharmProjects/ThermalModelling/synthetic_data/from_atlasz/newdata/turbulence_noise/noise=1.50/turbulence=0.6/5/decomposition/average/0/final/reconstructed/ground_truth_reconstructed


air_component = 'air'
average_removed=False
base_path = Path(os.path.join(root_path, 'synthetic_data/from_atlasz/newdata/turbulence_noise'))
glob_string = 'noise=*/turbulence=0.0/*/decomposition/average/0/final/reconstructed'
save_folder = os.path.join(root_path, 'results/turbulence/turbulence_noise/mesoscale_new_centers', air_component)
save_filename = f'turbulence_correlation_{grid_size=}_no_turbulence.csv'
if not average_removed:
    save_filename = save_filename.replace('.csv', '_with_average.csv')
if save:
    os.makedirs(save_folder, exist_ok=True)
# df_all_correlations = pd.read_csv(os.path.join(save_folder, save_filename), index_col=False)

if os.path.exists(os.path.join(save_folder, save_filename)) and (not overwrite):

    df_all_thermals = pd.read_csv(os.path.join(save_folder, save_filename), index_col=False)
    df_all_thermals_avg = pd.read_csv(os.path.join(save_folder, save_filename.replace('.csv', '_avg_per_realization.csv')),
                                  index_col=False)
else:
...
for i_t, current_path in enumerate(base_path.glob(glob_string)):
    for datatype in ['dec', 'gt']:
        ...
        if ((not df_all_thermals_avg.empty)
                and (not df_all_thermals_avg[np.all(df_all_thermals_avg[list(label_dict.keys())].astype(float) == [float(v) for v in label_dict.values()],
                                                    axis=1)
    ...
    df_all_thermals_avg = df_all_thermals.groupby(list(label_dict.keys())
                                                  + ['datatype', 'size', 'delta_R']
                                                  ).agg(**{f'{col}_{stat}': (col, stat)
                                                           for col in ['inner' ,'x' ,'y' ,'z']
                                                           for stat in ['mean', 'median', 'std', 'count', 'sem']
                                                           }).reset_index()
    ...
    if save:
        df_all_thermals_avg.to_csv(os.path.join(save_folder, save_filename.replace('.csv', '_avg_per_realization.csv')),
                                        index=False)
