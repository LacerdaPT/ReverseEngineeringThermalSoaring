...
for air_component in ['air', 'thermal']:
    path_to_csvs = os.path.join(root_path, f'results/turbulence/storks/small_scale', air_component)
    ...
    df_summary = pd.read_csv(f'results/air_velocity_field/storks/thermals_summary.csv', index_col=False)
    ...
    if save:
        df_summary.to_csv(f'results/air_velocity_field/storks/thermals_summary_{air_component}_fluctuations.csv', index=False)
        ...
        for i_col2, col2 in enumerate(my_cols[i_col1+1:]):
            for s in np.sort(df_summary['radius'].unique()):
                ...
                pearson_result = stats.pearsonr(df_summary.loc[df_summary['radius'] == s, col1],
                           df_summary.loc[df_summary['radius'] == s, col2], method=PermutationMethod(10000, batch=9999))
