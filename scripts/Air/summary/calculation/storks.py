...
save_folder = os.path.join(root_path, f'results/air_velocity_field/storks/')
if save:
...
for i_t, current_path in enumerate(base_path.glob(glob_string)):
    ...
    if (not df_sigmas.empty) and (not df_sigmas[np.all(df_sigmas[label_keys].astype(str) ==[str(label_dict[k]) for k in label_keys], axis=1)].empty):
        continue
...
df_stats['thermal'] = df_stats['thermal'].apply(lambda x: stork_dataset_renaming_dict[x])
